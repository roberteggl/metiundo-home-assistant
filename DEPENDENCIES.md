...
**Also defined in:** `custom_components/metiundo/manifest.json`
...
- `basedpyright` - Type checker (we prefer basedpyright over HA's mypy for better IDE integration)
- `mdformat` (+ `mdformat-gfm`, `mdformat-footnote`) - Markdown formatter
...
**Version:** Derived from the minimum Home Assistant version in `hacs.json`; bootstrap installs the latest patch release in that train.
...
2. **Test dependencies** (`requirements_test.txt`)
   ...
   - mypy-dev for type checking (we use basedpyright instead)
...
- `requirements_dev.txt` - Additional development tools this project uses (basedpyright, mdformat, pymarkdownlint, colorlog, performance packages)
