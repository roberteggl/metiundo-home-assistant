...
- `custom_components/metiundo/` - Main integration code
...
- Integration logs: `custom_components.metiundo: debug` in `config/configuration.yaml`
- You can modify log levels when debugging - just restart HA after changes

**Context-specific instructions:**

Path-specific instructions in `.github/instructions/*.instructions.md` provide additional guidance for specific file types (Python, YAML, JSON, etc.). This document serves as the primary reference for all agents.
...
**For workflow basics (small changes, translations, tests, session management):** See this document's guidance on workflow and session management below.
...
- `.github/instructions/blueprint.commit-message.instructions.md` - Conventional Commits format and conventions

**For detailed workflow guidance:** See `.github/instructions/blueprint.workflow.instructions.md` if present, or this document's guidance below.
...
- **Domain:** `metiundo`
- **Title:** Metiundo Smart Meter
- **Class prefix:** `Metiundo`

**When creating new files:**

- Use the domain `metiundo` for all DOMAIN references
- Prefix all integration-specific classes with `Metiundo`
- Use "Metiundo Smart Meter" as the display title
...
- Inherit from platform base + `MetiundoEntity`
...
script/type-check   # basedpyright type errors — no auto-fix ever, always a manual loop
...
script/markdown     # mdformat + pymarkdownlint        (Markdown only)
```
...
script/markdown-check # mdformat --check + pymarkdownlint (Markdown only)
script/type-check   # basedpyright                     (types only)
...
| **basedpyright**   | Python type checking         | ❌ manual            |
...
| **mdformat**       | Markdown formatting          | ✅ `script/markdown` |
| **pymarkdownlint** | Markdown structure + style   | ✅ `script/markdown` |
| **codespell**      | Spelling in code + docs      | ✅ `script/spell`    |
| **pytest**         | Unit + integration tests     | ❌ n/a               |

References: [Ruff rules](https://docs.astral.sh/ruff/rules/) · [basedpyright docs](https://docs.basedpyright.com/)
...
For basedpyright type errors run `script/type-check` — there is no auto-fix for type errors ever.
...
- `tests/` mirrors `custom_components/metiundo/` structure
...
- [basedpyright Configuration](https://docs.basedpyright.com/) - Type checking options
...
- [basedpyright Configuration](https://docs.basedpyright.com/) - Type checker documentation
